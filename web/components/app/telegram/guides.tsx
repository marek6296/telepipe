...
export function GuideList({ children }: { children: ReactNode }) {
  return (
    <ol className="space-y-2.5 text-[13px] leading-relaxed text-white/55">{children}</ol>
  );
}
...
export function GuideStep({ n, children }: { n: number; children: ReactNode }) {
  return (
    <li className="flex gap-3">
      <span className="mt-px flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-[rgba(212,175,55,0.3)] bg-[rgba(212,175,55,0.08)] text-[10.5px] font-semibold text-[var(--gold-light)]">
        {n}
      </span>
...
export function GuideLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1 font-medium text-[var(--gold-light)] underline decoration-[rgba(212,175,55,0.35)] underline-offset-2 transition-colors hover:decoration-[var(--gold)]"
    >
      {children}
  ...
  return (
    <GuideList>
      <GuideStep n={1}>
        ...
        in with <strong className="font-medium text-white/75">her</strong> phone number —
        Telegram sends a confirmation code to that account.
      </GuideStep>
      <GuideStep n={2}>
        Click <strong className="font-medium text-white/75">API development tools</strong>.
      </GuideStep>
      <GuideStep n={3}>
      ...
      <GuideStep n={4}>
        Copy <code className="rounded bg-white/[0.06] px-1 py-0.5 text-[12px]">api_id</code>{" "}
        (a number) and{" "}
        <code className="rounded bg-white/[0.06] px-1 py-0.5 text-[12px]">api_hash</code> (32
  ...
  return (
    <GuideList>
      <GuideStep n={1}>
        ...
        send <code className="rounded bg-white/[0.06] px-1 py-0.5 text-[12px]">/newbot</code>.
      </GuideStep>
      <GuideStep n={2}>
        Give it any name, then a username that ends with{" "}
        <strong className="font-medium text-white/75">bot</strong>.
      </GuideStep>
      <GuideStep n={3}>
      ...
      <GuideStep n={4}>
        Open your new bot and press <strong className="font-medium text-white/75">Start</strong>
  ...
  return (
    <GuideList>
      <GuideStep n={1}>
        ...
        <strong className="font-medium text-white/75">your own</strong> Telegram account and
        press Start.
      </GuideStep>
      <GuideStep n={2}>
